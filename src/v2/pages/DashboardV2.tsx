...
const DashboardV2 = () => {
  ...
  // Map V2 templates to profession categories
  // For now, we'll show V2 templates in the "All Templates" category
  // and create a mapping for future templates
  const getV2TemplateCount = (categoryId: string) => {
    if (categoryId === 'all') {
      return v2Templates.length;
    }
    // Map V2 category to profession category
    // For now, 'professional' maps to 'business' or 'all'
    const categoryMap: Record<string, string> = {
      'business': 'professional',
      'technology': 'professional',
      'design': 'professional',
    };
    const v2Category = categoryMap[categoryId] || 'professional';
    return getTemplatesByCategory(v2Category as any).length;
  };

  const handleCategoryClick = (categoryId: string) => {
    // Navigate to V2 profession templates page (same as V1)
    navigate(`/v2/${categoryId}`);
  };
        ...
        <div className="mb-8 md:mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              ...
              {(() => {
                ...
                const v2TemplateCount = getV2TemplateCount('all');
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {professionCategories
              ...
              .map((category) => {
                ...
                const v2TemplateCount = getV2TemplateCount(category.id);
