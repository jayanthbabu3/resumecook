...
const UserRow: React.FC<{
  ...
  return (
    <div className="flex items-center gap-3 p-3 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0">
      <Avatar className={cn('w-8 h-8', user.subscription?.status === 'active' && 'ring-1 ring-primary/30')}>
        <AvatarImage src={user.profilePhoto} alt={user.fullName} />
        <AvatarFallback className={cn('text-xs font-semibold text-white', user.subscription?.status === 'active' ? 'bg-gradient-to-br from-primary to-blue-600' : 'bg-gray-400')}>
...
export const AdminUsersPage: React.FC = () => {
    ...
    const loadUsers = async () => {
      try {
        const response = await adminService.getUsers(1, 100, {
          status: subscriptionFilter === 'all' ? undefined : subscriptionFilter as any,
  ...
  const counts = users.reduce(
    ...
    { all: 0, active: 0, trial: 0, none: 0 } as Record<string, number>
  );
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{counts.all} total • {counts.active} pro</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          ...
          <StatsCard title="Pro" value={counts.active} icon={<Crown className="w-4 h-4 text-purple-600" />} color="bg-purple-100" />
          <StatsCard title="Trial" value={counts.trial} icon={<Clock className="w-4 h-4 text-amber-600" />} color="bg-amber-100" />
          <StatsCard title="Free" value={counts.none} icon={<UserCheck className="w-4 h-4 text-green-600" />} color="bg-green-100" />
        </div>
