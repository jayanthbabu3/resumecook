/**
 * Professional Compact Template Component
 *
 * Clean professional resume with integrated header summary
 * and accent-colored contact bar.
 */
...
const ProfessionalCompactContent: React.FC<Omit<TemplateComponentProps, 'config'>> = (props) => {
  const { config, resumeData, editable } = useBaseTemplate();
  const { spacing, colors, fontFamily, typography } = config;

  const allSections = useOrderedSections();

  // Filter out summary section - it's integrated into the header for this template
  // This MUST always filter out summary regardless of user's enabledSections settings
  const sections = React.useMemo(() => {
    return allSections.filter(s => s.type !== 'summary');
  }, [allSections]);
  ...
  return (
    <div style={containerStyle}>
      <div style={contentStyle}>
        {/* Header - With integrated summary and accent contact bar */}
        <HeaderSection
...
export const ProfessionalCompactTemplate: React.FC<TemplateComponentProps> = (props) => {
  return (
    <BaseTemplateProvider {...props}>
      <ProfessionalCompactContent {...props} />
    </BaseTemplateProvider>
  );
};
